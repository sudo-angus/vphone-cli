...
class VPhoneKeyHelper {
    ...
    /// True when a hardware keyboard device is attached to the VM.
    /// Required for `typeString` — the software-keyboard mode boots the VM
    /// without a `_VZKeyboard` and there is nothing to receive key events.
