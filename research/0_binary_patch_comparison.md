...
- Runtime validation blocker observed on 2026-03-10:
  ...
  - `amfidont` was then validated on-host:
    ...
    - amfidont is fragile across `amfid` being launchd-recycled: its lldb session sees the target in `eStateExited` (state 10), raises `RuntimeError: Unexpected process state 10`, and the worker exits, silently dropping the bypass. `scripts/amfidont_supervisor.sh` now wraps the worker and relaunches it on recycle. Both `start_amfidont_for_vphone.sh` (root supervisor) and the manager (`VPhonePrivilege`, user supervisor via the scoped NOPASSWD rule — no re-auth) route through it; the manager also treats the recycle race as success-pending so it no longer surfaces a spurious "Could not start amfidont" alert.
